...
int game::Texture::addTexture(unsigned int index, std::string file_path) {
...
int game::Texture::bind(unsigned int index) {
    if (index < 0 || index >= n) {
        return 0;
    }
    glBindTexture(GL_TEXTURE_2D, textures[index]);
...
game::TEXTURE_ERROR game::Texture::makeGameTextures() {
    if (n != NUM_TEXTURES) {
        return IN_COMPATIBLE_SIZE;
    }
